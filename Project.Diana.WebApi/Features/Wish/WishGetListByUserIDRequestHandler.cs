...
namespace Project.Diana.WebApi.Features.Wish
{
    public class WishGetListByUserIDRequestHandler : IRequestHandler<WishGetListByUserIDRequest, WishListResponse>
    {
        ...
        public async Task<WishListResponse> Handle(WishGetListByUserIDRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                AlbumWishes = wishes.Where(w => w.ItemType == ItemReference.Album).GroupBy(g => g.Category).Select(list => new WishList { Category = list.Key, Wishes = list.OrderBy(x => x.Title) }),
                BookWishes = wishes.Where(w => w.ItemType == ItemReference.Book).GroupBy(g => g.Category).Select(list => new WishList { Category = list.Key, Wishes = list.OrderBy(x => x.Title) }),
                GameWishes = wishes.Where(w => w.ItemType == ItemReference.Game).GroupBy(g => g.Category).Select(list => new WishList { Category = list.Key, Wishes = list.OrderBy(x => x.Title) }),
                MovieWishes = wishes.Where(w => w.ItemType == ItemReference.Movie).GroupBy(g => g.Category).Select(list => new WishList { Category = list.Key, Wishes = list.OrderBy(x => x.Title) })
            };
